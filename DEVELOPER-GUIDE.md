...
This fork uses a layered dependency model:

- **Sub-package runtime deps** live in each sub-package's `setup.cfg` under
  `install_requires`. Edit those when a sub-package's library code starts
  using a new third-party module.
...
- **Dev tooling** (pytest, mypy, black, sphinx, pip-tools) lives in
  `requirements-dev.in`.
- The fully-pinned, transitively-resolved set lives in `requirements.lock`,
  generated by `pip-compile` from the three sources above.

The Dockerfile installs from `requirements.lock` when it exists, else falls
back to the `.in` files for bootstrapping a clean clone.
...
Sub-package `install_requires` use loose lower-bound pins (e.g. `numpy >= 1.23,
< 2.0`). The lockfile pins the exact resolved versions. This gives us both
"sub-packages are installable independently with relaxed constraints" and
"the lab build is byte-reproducible".

The `[options.extras_require]` block in `abides-gym/setup.cfg` declares
optional extras (`rllib`, which pulls Ray and Weights & Biases). Install with
`pip install -e ./abides-gym[rllib]` for users who want the RL training stack.
